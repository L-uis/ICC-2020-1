...
public class Matriz {
    ...
    public Matriz eliminarFila(int k) {
        Matriz resultado = new Matriz(m-1,n);
        ...
        for (int i = 0; i < m ; i++ ) {
            ...
            if (i > k) {
                for (int j = 0; j < n; j++ ) {
                    resultado.datos[i-1][j]=datos[i][j];
                }
            }
        }
        return resultado;
    ...
    public Matriz eliminarColumna(int k) {
        Matriz resultado = new Matriz(m,n-1);
        for (int i = 0; i < m; i++ ) {
            for (int j = 0; j < n ; j++ ) {
                if (j < k) {
                    resultado.datos[i][j] = datos[i][j];
                ...
                if (j > k) {
                    resultado.datos[i][j-1] = datos[i][j];
                }
            }
        }
        return resultado;
